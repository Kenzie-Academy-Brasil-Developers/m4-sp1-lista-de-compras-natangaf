...
const validateKeysList = (request: Request<any, any, iBody>, response: Response, next: NextFunction): Response | void => {
    ...
    if (typeof body.listName == "number") {
        return response.status(404).json({ mensage: `Name not Valid : ${body.listName}` })
    }
    for (const key in body) {
        if (!requiredKeys.includes(key)) {
            return response.status(404).json({ mensage: `Requered Keys Are : ${key}` })
        }
        if (!body.hasOwnProperty("listName") || !body.hasOwnProperty("data")) {
            return response.status(404).json({ mensage: `Requered Keys Are : ${requiredKeys}` })
        }
    }
